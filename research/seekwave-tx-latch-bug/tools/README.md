...
Firmware blobs live in `firmware/common/seekwave-fw/` (also `/lib/firmware/` on a running box).
...
For DRAM use `-loader-baseAddr 0x20200000` (**not** `0x20000000` — pointer-scoring suggests that and
is wrong; the correct base is anchored on the rate table at `0x20217CE0`).
...
| `idle-repro.sh <secs…>`        | hold the link idle/trickle after associating, _then_ load — the low-offered-load trigger |
